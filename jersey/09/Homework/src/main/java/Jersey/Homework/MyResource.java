package Jersey.Homework;

import java.util.HashMap;
import java.util.Map;
import java.util.Map.Entry;

import javax.ws.rs.DELETE;
import javax.ws.rs.GET;
import javax.ws.rs.PUT;
import javax.ws.rs.Path;
import javax.ws.rs.Produces;
import javax.ws.rs.QueryParam;
import javax.ws.rs.core.MediaType;
import javax.ws.rs.core.Response;

/**
 * Root resource (exposed at "/JerseyHomework" path)
 */
@Path("/JerseyHomework")
public class MyResource {
	
	private static Map<String, String> data = new HashMap<String, String>();
	
    @GET
    @Produces(MediaType.TEXT_PLAIN)
    public String getIt() {
        String output = new String();
        
        for(Entry<String, String> item : data.entrySet()) {
        		output += item.getKey() + ":" + item.getValue() + "\n";
        }
        
        return output;
    }
    
    @PUT
    public Response addElement(@QueryParam("key") String key, @QueryParam("value") String value) {
    		if (key.isEmpty()) {
    			System.out.println("Empty key");
    			return Response.status(204).entity("Please include a key").build();
    		}
    	
    		data.put(key, value);
    		
    		return Response.status(200).entity("Add successful").build();
    }
    
    @DELETE
    public Response removeElement(@QueryParam("key") String key) {
    		if (key.isEmpty()) {
    			System.out.println("Empty key");
			return Response.status(204).entity("Please include a key").build();
		}
    	
    		data.remove(key);
    		
    		return Response.status(200).entity("Remove successful").build();
    }
}
